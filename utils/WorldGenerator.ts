...
/**
 * Spawns a single, specific vignette at a target location.
 * Used by the KillerSystem to dynamically add clues to the world.
 * @param vignetteId The ID of the vignette to spawn (e.g., 'crime_stabbing')
 * @param x The target X coordinate.
 * @param y The target Y coordinate.
 * @returns An array of new Decal entities.
 */

export const spawnVignette = (vignetteId: string, x: number, y: number): Entity[] => {
  const newDecals: Entity[] = [];
  const chosenVignette = VIGNETTES.find(v => v.id === vignetteId);

  if (!chosenVignette) {
    console.error(`[KillerSystem] Could not find vignette with ID: ${vignetteId}`);
    return [];
  }

  const uniqueId = `killer_act_${Date.now()}`;

  chosenVignette.items.forEach(item => {
    newDecals.push({
      id: `${uniqueId}_${item.idSuffix}`,
      type: 'prop',
